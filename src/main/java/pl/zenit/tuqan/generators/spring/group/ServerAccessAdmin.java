...
class ServerAccessAdmin extends FileGenerator {

    public ServerAccessAdmin(TuqanExecutionParameters params) {
        super(params);
    }
    ...
    @Override public void createAt(File outputDir) throws IOException {
        ...
        text.add("public @interface AdminAccess {");
        text.add("}");

        Files.write(new File(outputDir, "AdminAccess.java").toPath(), text.getAsStringList());
    }
}
