...
class ServerAccessUser extends FileGenerator {

    public ServerAccessUser(TuqanExecutionParameters params) {
        super(params);
    }
    ...
    @Override public void createAt(File outputDir) throws IOException {
        ...
        text.add("public @interface UserAccess {");
        text.add("}");

        Files.write(new File(outputDir, "UserAccess.java").toPath(), text.getAsStringList());
    }
}
