...
public class SolidityEnumOutputGenerator extends OutputGenerator {

    public SolidityEnumOutputGenerator(TuqanExecutionParameters params) {
        super(params);
    }
    ...
    private void output(DefaultEnum enumData) throws GenerationFuckup {
        ...
        File solidityFile = getFileFor(enumName + ".sol");
        FilesaveWrapper.saveToFile(fileText.getAsStringList(), solidityFile.getAbsolutePath());
    }
    ...
    private void addEnumDefinition(IndentedStringList text, DefaultEnum enumData) throws GenerationFuckup {
        ...
        text.add("");
        text.add("contract " + enumData.name() + "Contract {");
        text.pushIndent();
    ...
    protected File getFileFor(String filename) {
        ...
        File targetDir = new File(dtoDir, DtoDirNames.solidity);
