...
public class SwiftEnumOutputGenerator extends OutputGenerator {

    public SwiftEnumOutputGenerator(TuqanExecutionParameters params) {
        super(params);
    }
    ...
    private void output(DefaultEnum enumData) throws GenerationFuckup {
        ...
        File swiftFile = getFileFor(enumName + ".swift");
        FilesaveWrapper.saveToFile(fileText.getAsStringList(), swiftFile.getAbsolutePath());
    }
    ...
    private void addEnumDefinition(IndentedStringList text, DefaultEnum enumData) throws GenerationFuckup {
        ...
        for (String value : enumValues) {
            text.add("case " + value.toLowerCase());
        }
    ...
    protected File getFileFor(String filename) {
        ...
        File targetDir = new File(dtoDir, DtoDirNames.swift);
